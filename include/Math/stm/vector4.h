...
namespace stm
{
	...
	public:
		...
		constexpr vector(std::span<T, 4> values) noexcept
		{
		...
		{
			return stm::norm(x) + stm::norm(y) + stm::norm(z) + stm::norm(w);
		}
		...
		constexpr vector<U, 3> cast() const noexcept
		{
			...
			{
				if constexpr (Real<T>)
					return { static_cast<Uu>(x) , static_cast<Uu>(y) , static_cast<Uu>(z) , static_cast<Uu>(w) };
				else
					return { U{static_cast<Uu>(x)} , U{static_cast<Uu>(y)} , U{static_cast<Uu>(z)} , U{static_cast<Uu>(w)}};
			}
		...
		operator std::span<T, N>() const noexcept { return { data() }; }
	...
	template<Number T>
	inline constexpr T dot(const vector<T, 4>& lhs, const vector<T, 4>& rhs) noexcept
	{
		return lhs.dot(rhs);
	}

	template<Number T>
	inline constexpr auto unit(const vector<T, 4>& vec) noexcept
	{
		return vec.unit();
	}

	template<Number T>
	inline constexpr T length(const vector <T, 4>&vec) noexcept
	{
		return vec.abs();
	}
